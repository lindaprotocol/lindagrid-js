const {assert, assertThrow, lindaGridBuilder, net} = require('../../helpers/includes');
    ...
    describe('#asset next page test', function () {
        ...
        it('should get all assets with pagination', async function () {
            for (let type of types) {
                let res = await lindaGrid.asset.getAll({limit: 200});
                let i = 1;
                while (res.meta && res.meta.fingerprint) {
                    res = await lindaGrid.nextPage(type === 'links' ? res.meta.links.next : res);
                    ++i;
                }
                assert.isTrue(i >= 1);
            }
        ...
        it('should get transactions with pagination', async function () {
            for (let type of types) {
                for (let address of addresses) {
                    let res = await lindaGrid.account.getTransactions(address);
                    let totalPage = 3;
                    let i = 1;
